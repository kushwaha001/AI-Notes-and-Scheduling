...
function Meter({ label, pct, detail }) {
  ...
  return (
    <div style={{ background: "var(--surface)", borderRadius: "14px", padding: "16px", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
      ...
      {detail && <p style={{ margin: "8px 0 0", color: "var(--muted)", fontSize: "12px" }}>{detail}</p>}
    </div>
  );
}
...
const STATUS_COLOR = {
  ok           : { bg: "var(--ok-soft)", color: "var(--ok)", label: "Online" },
  unreachable  : { bg: "var(--danger-soft)", color: "var(--danger)", label: "Offline" },
  error        : { bg: "var(--warn-soft)", color: "var(--warn)", label: "Error" },
  configured   : { bg: "var(--accent-soft)", color: "var(--accent)", label: "Configured" },
};
...
export default function SystemStatusPage() {
  ...
  return (
    <>
      ...
      {lastRefresh && <p style={{ color: "var(--muted)", fontSize: "13px", marginBottom: "24px" }}>Last refreshed: {lastRefresh}</p>}

      {/* Service health */}
      <h2 style={{ marginBottom: "14px" }}>AI &amp; Infrastructure Services</h2>
      {loading && <p style={{ color: "var(--muted)" }}>Checking services…</p>}
      {services && (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "12px", marginBottom: "32px" }}>
        <div style={{ background: "var(--surface)", borderRadius: "14px", padding: "16px", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
          ...
          <p style={{ margin: "4px 0 0", color: "var(--muted)", fontSize: "12px" }}>{sys?.model?.model || "—"}</p>
        </div>

        <div style={{ background: "var(--surface)", borderRadius: "14px", padding: "16px", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
          <p style={{ margin: 0, fontWeight: 600, fontSize: "14px" }}>Queue depth</p>
          <p style={{ margin: "8px 0 0", fontSize: "26px", fontWeight: 700, color: "var(--accent)" }}>{sys?.queue?.depth ?? "—"}</p>
          <p style={{ margin: "4px 0 0", color: "var(--muted)", fontSize: "12px" }}>
      ...
      <div style={{
        background: "var(--surface)", borderRadius: "16px", padding: "18px 22px", marginBottom: "32px",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "24px" }}>
        {[
          { label: "Waiting",           key: "waiting",          bg: "var(--warn-soft)", color: "var(--warn)" },
          { label: "Processing",        key: "processing",       bg: "var(--accent-soft)", color: "var(--accent)" },
          { label: "Awaiting Confirm",  key: "awaiting_confirm", bg: "var(--surface-2)", color: "var(--text-2)" },
          { label: "Failed",            key: "failed",           bg: "var(--danger-soft)", color: "var(--danger)" },
          ...
          <div key={key} style={{ background: bg, borderRadius: "14px", padding: "16px" }}>
            ...
            <p style={{ margin: "4px 0 0", color: "var(--muted)", fontSize: "13px" }}>{label}</p>
          </div>
        ))}
      </div>

      {queue.length > 0 && (
        <div style={{ background: "var(--surface)", borderRadius: "18px", padding: "20px", boxShadow: "0 4px 16px rgba(0,0,0,0.06)", marginBottom: "32px" }}>
          ...
          {queue.slice(0, 8).map((job) => (
            <div key={job.id} style={{
              ...
              <span style={{
                ...
                background: job.status === "failed" ? "var(--danger-soft)" : job.status === "waiting" ? "var(--warn-soft)" : "var(--ok-soft)",
      ...
      <div style={{ background: "var(--surface)", borderRadius: "18px", padding: "20px", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
        {audit.length === 0 ? (
          ...
          audit.map((entry) => (
            <div key={entry.id} style={{
              ...
              <span style={{ color: "var(--muted)", fontSize: "12px", whiteSpace: "nowrap", marginLeft: "16px" }}>

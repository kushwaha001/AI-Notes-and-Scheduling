...
export default function TasksPage() {
  ...
  const [form, setForm] = useState({ title: "", due_date: "", start_time: "", end_time: "", category: "General", priority: "Medium", recurrence: "", count: "" });
  ...
  async function handleCreate() {
    if (!form.title?.trim() || !form.due_date || !form.start_time || !form.end_time) {
      setMsg("Error: All fields are mandatory. Please fill in the Title, Due date, Start Time, and End Time.");
      return;
    }
    ...
    try {
      ...
      setForm({ title: "", due_date: "", start_time: "", end_time: "", category: "General", priority: "Medium", recurrence: "", count: "" });
      setShowForm(false);
      loadTasks();
    } catch (e) {
  ...
  async function handleUpdateTask() {
    ...
    try {
      await updateTask(editTask.id, {
        ...
        start_time: editTask.start_time || null,
        end_time: editTask.end_time || null,
    ...
    <div style={{ maxWidth: 980 }}>
      ...
      <div style={{ marginBottom: 22 }}>
        ...
        {showForm && (
          <div style={{ ...card, padding: 20, marginTop: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 0.8fr 0.8fr", gap: 12, marginBottom: 14 }}>
              <div>
              ...
              <div>
                ...
                <input type="time" value={form.end_time || ""}
                  onChange={(e) => setForm((f) => ({ ...f, end_time: e.target.value }))} style={inputStyle} />
              </div>
            </div>
      ...
      <div style={{ ...card, overflow: "hidden" }}>
        ...
        {tasks.map((task, i) => {
          ...
          >
            {isEditing ? (
              /* Inline edit — title / due date / priority / category / start & end times */
              <div>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                  ...
                  <div>
                    <label style={labelStyle}>Start Time</label>
                    <input type="time" value={editTask.start_time || ""} onChange={(e) => setEditTask((t) => ({ ...t, start_time: e.target.value }))}
                      style={{ ...inputStyle, width: "auto", cursor: "pointer" }} />
                  </div>
                  <div>
                    <label style={labelStyle}>End Time</label>
                    <input type="time" value={editTask.end_time || ""} onChange={(e) => setEditTask((t) => ({ ...t, end_time: e.target.value }))}
                      style={{ ...inputStyle, width: "auto", cursor: "pointer" }} />
                  </div>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", fontSize: 13, alignItems: "center" }}>
                ...
                {(task.due_date || task.start_time) && (
                  <span style={{ color: "var(--muted)" }}>
                    ...
                    {task.start_time ? ` @ ${String(task.start_time).slice(0, 5)}${task.end_time ? `–${String(task.end_time).slice(0, 5)}` : ""}` : ""}
                  </span>
                )}
                {task.classification && (

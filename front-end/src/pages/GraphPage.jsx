...
export default function GraphPage() {
  ...
  useEffect(() => {
    if (!data) return;
    if (layout === "timeline") {
      // Save current web coordinates before applying timeline layout
      ...
      // Switching back to web: restore coordinates from wx/wy
      let hasWebCoords = true;
      nodesRef.current.forEach((n) => {
      ...
      // If we don't have saved web coordinates, seed them on a circle
      if (!hasWebCoords) {
        const n = nodesRef.current.length || 1;
        nodesRef.current.forEach((nd, i) => {
          nd.x = W / 2 + Math.cos((2 * Math.PI * i) / n) * 320 + (i % 7) * 3;
          nd.y = H / 2 + Math.sin((2 * Math.PI * i) / n) * 320 + (i % 5) * 3;
          nd.vx = 0; nd.vy = 0;
        });
      }
      fitRef.current = true;
  ...
  function fitView(subset) {
    let ns = subset;
    if (!ns || !ns.length) {
      const baseNodes = nodesRef.current.filter((n) => filters[n.kind]);
      ns = layoutRef.current === "timeline" ? baseNodes.filter((n) => !n._out) : baseNodes;
      if (focusRef.current && layoutRef.current === "web") {
        const adj = buildAdj();
        const focusSet = new Set([focusRef.current, ...(adj[focusRef.current] || [])]);
        ns = ns.filter((n) => focusSet.has(n.id));
      }
    }
    // Filter out any nodes with invalid coordinates to prevent NaN propagation to the view state
    ns = ns.filter((n) => n && !isNaN(n.x) && !isNaN(n.y));
  ...
  const toGraph = (clientX, clientY) => {
    ...
    const sx = ((clientX - r.left) / width) * W;
    const sy = ((clientY - r.top) / height) * H;
    const k = view.k || 1;
    return { x: (sx - view.x) / k, y: (sy - view.y) / k };
  };

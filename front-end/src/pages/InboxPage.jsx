...
export default function InboxPage() {
  ...
  async function handleConfirmAll(item) {
    const list = details[item.job_id] || [];
    const missingMandatory = list.some((ex) => {
      const dateVal = ex.item_type === "event" ? ex.event_date : (ex.deadline || ex.reply_by || ex.event_date);
      return !ex.subject?.trim() || !dateVal || !ex.event_time || !ex.event_end_time;
    });
    if (missingMandatory) {
      alert("Some extracted items in this document are missing mandatory details (Title, Date, Start Time, or End Time). Please click Review to fill them in before confirming.");
      return;
    }
    setBusyJob(item.job_id);
    try {
